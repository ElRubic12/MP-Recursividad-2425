...
int main()
{
    int vector[TAM] = {1, 2, 8, 24, 2, 4};
    int resultado;

    resultado = funcionrara( vector, 2, 1, 1, vector[1]*vector[TAM-1] );
...
int funcionrara( int v[], int i, int alfa, int correcto, int suma )
{
    if( i > TAM/2 )     ///Caso base se ha recorrido la primera mitad del vector
    {
        ...
        if( i-1 >= alfa && v[i] == suma )       ///Caso base para el sumatorio
        {
            if( i-1 < alfa+1 )                  ///Caso en el que se llegue al final del sumatorio
            {
                return funcionrara( v, i+1, 1, 0, suma+=(v[alfa+1] * v[TAM - alfa-1]) );
            ...
            return funcionrara( v, TAM/2+1, 1, 1, suma );
        }
